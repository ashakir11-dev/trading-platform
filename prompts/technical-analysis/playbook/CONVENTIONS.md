...
- All numbers are [C] house defaults unless a topic file labels them otherwise. role.md rules, field names and limits stay authoritative (sessions; 1 week = 5 sessions).
...
- P pivot, b buffer, e entry, s stop, R = e - s, T1 = role.md `targets[0]`, CP1/CP2 checkpoints, E expected bars to T1, HEM/HEMmax (09 2.2).
...
| Role earnings scan | `as_of` to `max_hold_until` | same | same | same |
