...
| Debt and going concern | `GetDebtProfile`, `GetGoingConcernStatus` | Only rows with a filing date (or the filing they come from) on an earlier day than `as_of`; otherwise a gap. |
...
| Never | (none) | `ScreenStocks`, `GetValuationMultiples`, `GetAnalystEstimates`, `GetUpcomingInvestorEvents`, `GetEtfProfile`, `GetLatestEconomicIndicators` answer "now": record each as a gap if the stage wants it. |
